...
class City(TableMethods):
    def __init__(self, city_name: str, county_name: str) -> None:
        parent_table = 'County'
        table_field = 'county_name'
        super().__init__(city_name, county_name, parent_table, table_field)


class Street(TableMethods):
    def __init__(self, street_name: str, city_name: str) -> None:
        parent_table = 'City'
        table_field = 'city_name'
        super().__init__(street_name, city_name, parent_table, table_field)
...
class House:
    ...
    def get_city_id(self, con: sqlite3.Connection):
        cursor = con.cursor()
        _id = ''
        for row in cursor.execute(f"""select id from City where city_name = '{self.parent_city_name}'"""):
            _id = row[0]
...
def read_data(
    ...
    for row in cursor.execute("""select distinct city, county from import_data"""):
        city_list.append(City(row[0], row[1]))

    for row in cursor.execute("""select distinct streetAddress, city from import_data"""):
...
def import_data(
    ...
    for entry in house_list:
            cursor.execute(f"""insert into House values (null, {entry.get_street_id(con)}, {entry.get_city_id(con)}, 
    ...
    for entry in sale_announcement_list:
        ...
        {entry.bank_owned}, {entry.new_build}, {entry.pets}, {entry.get_home_type_id(con)})""")

    con.commit()


if __name__ == '__main__':
    connection = sqlite3.connect('./project_database.sqlite')
